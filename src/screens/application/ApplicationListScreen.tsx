...
export const ApplicationListScreen: React.FC<TProps> = props => {
  ...
  const deleteData = (pk: number) => {
    setLoading(true);
    const request = async () => {
      await ApplicationService.delete(pk);
    };
    request();
  };
    ...
    <Root>
      <View style={{ flex: 1}}>
        {isLoading ? <ActivityIndicator style={{marginTop: 16}} size="large" /> : (
          <View style={{ flex: 1}}>
            <Text style={{fontSize: 26, textAlign: 'center'}}>Applications</Text>
            <FlatList
              data={data}
              extraData={refresh}
              keyExtractor={item => String(item.id)}
              renderItem={({ item }: { item: ApplicationModel }) => (
                <View style={styles.container}>
                  <View style={styles.itemContainer}>
                    ...
                    <View >
                      <TouchableOpacity
                        onPress={() =>
                          navigation.navigate(NavigationNames.ApplicationDetailScreen, {
                            application: JSON.stringify(item)
                          })
                        }
                      >
            ...
            >
              <TouchableOpacity
                ...
                onPress={() => 
                  navigation.navigate(NavigationNames.ApplicationDetailScreen, {application: JSON.stringify({id: 0, name: "", description: ""})})
                }
              >
